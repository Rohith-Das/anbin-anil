...
const placeOrder = async (req, res) => {
  try {
    ...
    const { shippingAddress, paymentMethod, razorpay_payment_id, razorpay_order_id, razorpay_signature, appliedCouponCode, useWallet } = req.body;
    ...
    if (useWallet && user.wallet && user.wallet.balance > 0) {
      const userWalletBalance = user.wallet.balance;
        ...
        { 
          ...
          $push: { 
            transactions: {
              ...
              type: 'debit',
              description: `Used for order ${generateOrderId()}`
            }
          }
        },
    ...
    if (paymentMethod === 'Razorpay' && totalAmount > 0) {
      ...
      if (isValidPayment) {
        ...
        return res.status(400).json({ success: false, message: 'Invalid Razorpay payment' });
      }
    } else if (paymentMethod === 'COD') {
      paymentStatus = 'Pending';
    } else if (paymentMethod === 'Wallet' || totalAmount === 0) {
      paymentStatus = 'Completed';
    }
    ...
    res.json({ 
      ...
      orderDetails: {
        ...
        walletAmountUsed: savedOrder.wallet_amount_used,
...
// Function to calculate refund amount for canceled items
const calculateRefundAmount = (order, canceledItems) => {
  ...
  // Calculate the proportion of the order that was canceled
  const canceledProportion = canceledSubtotal / originalSubtotal;

  // Calculate the coupon discount for the canceled items
  ...
  // If the entire order is canceled, refund any wallet amount used
